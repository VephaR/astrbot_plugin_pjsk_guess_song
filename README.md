...
插件内置了积分排行榜、每日游戏次数限制、游戏冷却等功能，并提供一个**轻量模式**选项以优化核心数较少服务器的表现。可通过API与远程服务器同步玩家分数和游戏数据。
...
- `重置题型统计`: 清空所有题型的统计数据（优先操作服务器数据）。
- `同步分数`: 将本地所有用户的总分强制同步至服务器。
...
```json
{
  ...
  "remote_resource_url_base": "https://your.server.com/path/to/resources",
...
- `remote_resource_url_base` (字符串): **远程资源URL**。当`use_local_resources`为`false`时，从此URL获取音频等资源。
- `stats_server_api_key` (字符串): **统计服务器API密钥**。用于连接后端服务器同步分数、排行榜和统计数据。留空则禁用所有在线功能。
